...
# FIRST PART 8 ASSIGNMENT
while True:
    if (cyprus.read_gpio() & 0b0001) == 1:
        cyprus.set_servo_position(1, 1)
        sleep(.1)
        print("not pressed = 180 degrees")
    elif (cyprus.read_gpio() & 0b0001) == 0:
        cyprus.set_servo_position(1, 0)
        sleep(.1)
        print("pressed = 0 degrees")
    else:
        print("Else")

# SECOND PART 8 ASSIGNMENT
import spidev
spi = spidev.SpiDev()
from pidev.Cyprus_Commands import Cyprus_Commands_RPi as cyprus
from pidev.stepper import stepper
...
for i in range(500, 1025, 25):
    cyprus.set_servo_position(1, i/1000)  # 2 specifies port P5, i is a float that specifies speed
    sleep(1)
    print(i)
cyprus.set_servo_position(1, 0.5)  # halt the motor
cyprus.close()  # when done disconnect the RPiMIB communication

# 2b PART 8 ASSIGNMENT
import spidev
spi = spidev.SpiDev()
from pidev.Cyprus_Commands import Cyprus_Commands_RPi as cyprus
from pidev.stepper import stepper
...
while True:
    if (cyprus.read_gpio() & 0b0001) == 1:
        cyprus.set_servo_position(1, .5) # port 4
        sleep(.1)
        print("not pressed = stopped")
    elif (cyprus.read_gpio() & 0b0001) == 0:
        cyprus.set_servo_position(1, 1)
        sleep(.1)
        print("pressed = full speed clockwise")
    else:
        print("Else")
...
sleep(5)
#turn it the other way on the board here

#3a PART 8 ASSIGNMENT
import spidev
spi = spidev.SpiDev()
from pidev.Cyprus_Commands import Cyprus_Commands_RPi as cyprus
from pidev.stepper import stepper
